...
public final class MapArtTileAccumulator {
    ...
    public static void handleTile(MapArtTilePayload payload, ServerPlayer player) {
        ...
        if (player.distanceToSqr(Vec3.atCenterOf(payload.pos())) > ModNetwork.REACH_SQUARED) {
        ...
        UpdateResult result = update(player.getUUID(), payload.pos(), player.level().dimension(),

...
public class MapArtGameTests {
    ...
    /** 26.2 の {@code GameTestHelper.fail} は {@code (Component, BlockPos)}。文字列は包んで渡す。 */
    private static void failAt(GameTestHelper helper, String message, net.minecraft.core.BlockPos pos) {
        helper.fail(net.minecraft.network.chat.Component.literal(message), pos);
    }

    private static MapArtMakerBlockEntity place(GameTestHelper helper) {
        helper.setBlock(POS, ModBlocks.MAP_ART_MAKER.get());
        if (!(helper.getLevel().getBlockEntity(helper.absolutePos(POS)) instanceof MapArtMakerBlockEntity maker)) {
            failAt(helper, "no block entity after placing the block", POS);
    ...
    public static void blockPlacesWithABlockEntity(GameTestHelper helper) {
        ...
        if (maker.getContainerSize() != MapArtMakerBlockEntity.SIZE) {
            failAt(helper, "container size " + maker.getContainerSize()
                    + " does not match " + MapArtMakerBlockEntity.SIZE, POS);
        }
        if (!maker.isEmpty()) {
            failAt(helper, "a freshly placed block should be empty", POS);
        }
        helper.succeed();
    }
    ...
    public static void storedMapKeepsItsColours(GameTestHelper helper) {
        ...
        if (!maker.consumeBlanksAndStore(1, 1, results)) {
            failAt(helper, "storing one tile should succeed with one blank map available", POS);
        }

        ItemStack stored = maker.getItem(1);
        if (stored.isEmpty()) {
            failAt(helper, "output slot 1 is empty after a successful store", POS);
        }
        MapItemSavedData data = MapItem.getSavedData(stored, level);
        if (data == null) {
            failAt(helper, "the stored item carries no map data", POS);
        } else if (!java.util.Arrays.equals(expected, data.colors)) {
            failAt(helper, "map colours differ from what was written", POS);
        }
        helper.succeed();
    }
    ...
    public static void blanksAreConsumed(GameTestHelper helper) {
        ...
        if (!maker.consumeBlanksAndStore(3, 1, results)) {
            failAt(helper, "a 3x1 strip should fit with four blanks and an empty grid", POS);
        }
        if (maker.blankCount() != 1) {
            failAt(helper, "expected one blank left, found " + maker.blankCount(), POS);
        }
        if (maker.freeOutputSlots() != MapArtMakerBlockEntity.OUTPUT_SLOTS - 3) {
            failAt(helper, "expected six free output slots, found " + maker.freeOutputSlots(), POS);
        }
        helper.succeed();
    }
    ...
    public static void finishedMapIsLocked(GameTestHelper helper) {
        ...
        if (data == null) {
            failAt(helper, "no map data on a freshly created tile", POS);
        } else if (!data.locked) {
            failAt(helper, "map art must be locked; MapItem's inventory tick repaints anything that is not", POS);
        }
        helper.succeed();
    }
    ...
    public static void tilesKeepTheirPositionInTheGrid(GameTestHelper helper) {
        ...
        if (!maker.consumeBlanksAndStore(2, 2, results)) {
            failAt(helper, "a 2x2 should fit in an empty grid with four blanks", POS);
        }
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if (data == null || !java.util.Arrays.equals(colours(i * 31), data.colors)) {
                failAt(helper, "tile " + i + " is not in slot " + slot
        ...
        for (int slot : new int[]{MapArtMakerBlockEntity.outputSlot(2, 0),
            ...
            if (!maker.getItem(slot).isEmpty()) {
                failAt(helper, "slot " + slot + " is outside a 2x2 and should be empty", POS);
            }
        }
        helper.succeed();
    }
    ...
    public static void refusesWithoutEnoughBlanks(GameTestHelper helper) {
        ...
        if (maker.consumeBlanksAndStore(2, 2, results)) {
            failAt(helper, "a 2x2 must not be produced from a single blank map", POS);
        }
        if (maker.blankCount() != 1) {
            failAt(helper, "a refused request must not spend blanks, found " + maker.blankCount(), POS);
        }
        if (maker.freeOutputSlots() != MapArtMakerBlockEntity.OUTPUT_SLOTS) {
            failAt(helper, "a refused request must not fill output slots", POS);
        }
        helper.succeed();
    }
    ...
    public static void refusesWhenOutputIsFull(GameTestHelper helper) {
        ...
        if (maker.consumeBlanksAndStore(1, 1, results)) {
            failAt(helper, "a full output area must refuse further tiles", POS);
        }
    ...
    public static void uploadedTilesReassembleAndStore(GameTestHelper helper) {
        ...
        if (!(result instanceof MapArtTileAccumulator.UpdateResult.Complete complete)) {
            failAt(helper, "four tiles of a 2x2 upload should complete the transfer", POS);
            return;
        }
        ...
        if (!maker.consumeBlanksAndStore(complete.tilesX(), complete.tilesY(), maps)) {
            failAt(helper, "a completed 2x2 upload should store cleanly with four blanks available", POS);
            return;
        }
        ...
        for (int i = 0; i < expected.length; i++) {
            ...
            if (data == null || !java.util.Arrays.equals(expected[i], data.colors)) {
                failAt(helper, "tile " + i + " lost its colours between upload and storage", POS);
            }
        }
        helper.succeed();
    }
